...
export const getLocations = async (): Promise<LocationsResponse> => new Promise<LocationsResponse>(
  (resolve, reject) => {
    ...
    msGraphInstance.graphClient.api('/sites/93c44b10-11ba-4212-8a79-bd8b60b8df73/lists/282e06a7-4e24-4a5c-a6da-bdd1f2263e63?select=id,name&expand=items(expand=fields(select=Country,Month,Year,Normaltiming,id,test_x0020_month))')
      .get()
...
export const getProjects = async (): Promise<ProjectsResponse> => new Promise<ProjectsResponse>(
  (resolve, reject) => {
    // eslint-disable-next-line max-len
    msGraphInstance.graphClient.api('/sites/93c44b10-11ba-4212-8a79-bd8b60b8df73/lists/7ef21964-9408-40b1-92ed-6f1d694b2b65?expand=items(expand=fields(select=Title,Relatessalesmanager,Company,id))')
      .get()
